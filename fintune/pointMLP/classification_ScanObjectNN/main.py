...
def parse_args():
    ...
    parser.add_argument('--test_bs', type=int, default=32, help='batch size in training')
...
def main():
    ...
    #     '/home/zhg/project/paper_project/checkpoint/pmlp_l/best_model.pth',
    ...
    # # net.module.embedding.net[0] = nn.Conv1d(6,32,1)
    # print("Load Model")

    if not os.path.isfile(os.path.join(args.checkpoint, "last_checkpoint.pth")):
        save_args(args)
        logger = Logger(os.path.join(args.checkpoint, 'log.txt'), title="ScanOBJ" + args.model)
    ...
    test_loader = DataLoader(ScanObjectNN(partition='test', num_points=args.num_points), num_workers=args.workers,
                             batch_size=args.test_bs, shuffle=True, drop_last=False)
